		...
		try {
			if(M360Constants.JSONFORMAT){
				...
				if(listNumber.getMessage360().getErrors().getError().size()!=0){
					...
					for(Phone curPhone:listNumber.getMessage360().getBuyNumber()){
						System.out.println("sid="+curPhone.getPhoneNumber()+",Sid:="+curPhone.getPhoneSid()+",status:="+curPhone.getStatus());
					}
				}
			}

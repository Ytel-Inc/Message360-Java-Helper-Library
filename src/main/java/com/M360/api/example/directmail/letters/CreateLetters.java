	...
	public static void main(String[] args) {
		...
		try {
			if(M360Constants.JSONFORMAT){
				String jsonSMSResponse=conn.createJsonLetters("{to}", "{from}", "{attachbyid}", "{file}", true, ExtraService.CERTIFIED, false, "{description}", true, "{htmldata}");
				System.out.println(jsonSMSResponse);
			}else{
				Message360<DMLetter> createLetter = conn.createLetters("{to}", "{from}", "{attachbyid}", "{file}", true, ExtraService.CERTIFIED, false, "{description}", true, "{htmldata}");
				if(createLetter.getMessage360().getErrors().getError().size()!=0){
					...
					System.out.println("Create Letter Card");

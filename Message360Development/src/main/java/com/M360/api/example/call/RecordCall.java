	...
	public static void main(String[] args) {
		...
		try {
			...
			if(M360Constants.JSONFORMAT){
				String jsonCallResponse=conn.recordJsonCall("{CallSid}", Direction.OUT, true, timeLimit, "callBackUrl", AudioFormat.MP3);
				System.out.println(jsonCallResponse);
			}else{
				Message360<CallMessages> recordCall = conn.recordCall("{CallSid}", Direction.OUT, true, timeLimit, "callBackUrl", AudioFormat.MP3);
